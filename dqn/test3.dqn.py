...
if __name__ == "__main__":
    ...
    # ======================================================================
    agent = DQNAgent()
    # ★★★★★ 수정된 부분 1: episode_rewards 리스트 초기화 ★★★★★
    episode_rewards = []
    
    env, start, goal = generate_map_with_guaranteed_path(size=SIZE)
    
    print("생성된 '훈련용' 맵을 확인하세요. 창을 닫으면 학습이 시작됩니다.")
    plt.figure(figsize=(6, 6)); map_visual = np.stack([env.copy().astype(float)] * 3, axis=-1)
    ...
    # ======================================================================
    NUM_TESTS = 5
    print(f"\n{'='*25}\n훈련된 모델로 {NUM_TESTS}개의 새로운 맵 테스트 시작\n{'='*25}")
    ...
    for i in range(NUM_TESTS):
        ...
        for _ in range(MAX_STEPS * 2):
            ...
            action_tensor = agent.select_action(state_tensor, use_exploration=False)
            action_idx = action_tensor.item()
            dx, dy = actions[action_idx]; next_state = (state[0] + dx, state[1] + dy)
        ...
        print(f"최단 경로 길이 (BFS): {len(shortest_path) - 1 if shortest_path else '경로 없음'}")
        if agent_path[-1] == test_goal:
            print(f"에이전트 경로 길이: {len(agent_path) - 1}")
        else:
            print("에이전트가 목표에 도달하지 못했습니다.")
            
        plt.figure(figsize=(8, 8))
        final_visual = np.stack([test_env.copy().astype(float)] * 3, axis=-1)
        for pos in shortest_path: final_visual[pos] = [1, 0, 1]
        for pos in agent_path: final_visual[pos] = [1, 1, 0]
        final_visual[test_start] = [0, 1, 0]; final_visual[test_goal] = [0, 0, 1]
        plt.imshow(final_visual); plt.title(f"Test #{i+1} - Shortest(Magenta) vs Agent(Yellow)"); plt.axis('off'); plt.show()