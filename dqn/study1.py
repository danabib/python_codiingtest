...
# --- 환경 및 기본 설정 (is_path_possible, generate_map, actions, 하이퍼파라미터 등) ---
def find_shortest_path_bfs(env, start, goal):
    size = env.shape[0]
    queue = deque([start]); visited = {start}; parent = {start: None}
    while queue:
        r, c = queue.popleft()
        if (r, c) == goal:
            path = []; curr = goal
...
MAX_STEPS = 300; MAX_EPISODES = 7000
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# --- DQN 핵심 구성 요소 (DQN, ReplayMemory, DQNAgent 등 클래스 정의) ---
class ReplayMemory:
...
if __name__ == "__main__":
    ...
    for i_episode in range(MAX_EPISODES):
        ...
        for t in range(MAX_STEPS):
            ...
            state = next_state # is_wall_hit이 True면, state는 변하지 않은 채로 다음 스텝 진행
    ...
    # ★★★★★ 새로운 기능: 훈련된 모델을 .pt 파일로 저장 ★★★★★
    MODEL_PATH = "dqn_model.pt"
    torch.save(agent.policy_net.state_dict(), MODEL_PATH)
    print(f"\n훈련된 모델이 '{MODEL_PATH}' 경로에 저장되었습니다.")

    # 보상 그래프 시각화
    plt.figure(); plt.title('Training Rewards'); plt.xlabel('Episode'); plt.ylabel('Total Reward')
    plt.plot(episode_rewards); plt.savefig('dqn_rewards.png'); plt.show()