...
def generate_map_with_guaranteed_path(size, obstacle_prob=0.3):
...
class PathfindingGUI:
    def __init__(self, master):
        self.master = master
        master.title("DQN Pathfinding Simulator (Fast Ver.)")
    ...
    def load_model(self):
        MODEL_PATH = "dqn\\dqn_model_fast.pt"
        try:
    ...
    def _step(self):
        ...
        if (0 <= next_state_candidate[0] < SIZE and 0 <= next_state_candidate[1] < SIZE and self.env[next_state_candidate] == 0):
            self.state = next_state_candidate
        self.agent_path.append(self.state); self.step_count += 1
