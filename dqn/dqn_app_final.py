...
class PathfindingGUI:
    ...
    def load_model(self):
        # 훈련시킨 모델 파일명 (.pt)을 정확히 기입해주세요.
        MODEL_PATH = "dqn\\dqn_model_generalized.pt" # 또는 "dqn_model_fast.pt" 등
        try:
    ...
    def _step(self):
        ...
        # ★★★★★ 핵심 수정: '갇힘'을 감지하고 '플랜 B' 실행 ★★★★★
        if is_wall_hit:
            # DQN의 판단이 벽에 막혔을 경우, 휴리스틱(Heuristic)으로 '플랜 B'를 가동
            print(f"DQN 판단 막힘! 현재 위치({self.state})에서 휴리스틱 탐색 실행...")
            
            best_next_state = self.state # 일단 제자리에 머무는 것으로 초기화
            min_dist = np.sqrt((self.state[0] - self.goal[0])**2 + (self.state[1] - self.goal[1])**2)

            # 현재 위치에서 상/하/좌/우를 모두 시도해본다
            ...
            self.state = best_next_state # 가장 좋은 길로 한 칸 이동 (또는 제자리)
